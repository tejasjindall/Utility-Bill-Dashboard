...
public class tel_billpay extends JFrame implements ActionListener
{
...
count=0;
ResultSet rs;
rs=s11.executeQuery("select * from telephone where flat_no=" + Integer.parseInt(t1.getText()));
while(rs.next())
{
...
if(count1==1)
{
jp.showMessageDialog(this,"Your Telephone bill is Rs.="+Integer.parseInt(t11.getText()));
}
}
}
catch(Exception ee)
...
Connection con=DriverManager.getConnection("jdbc:odbc:bill_project");
Statement s11=con.createStatement();
s11.executeUpdate("update telephone set status='paid' where flat_no=" + Integer.parseInt(t1.getText()));
...
{
tel_billpay na=new tel_billpay();
}
}